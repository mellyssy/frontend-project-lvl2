...
test.each(['json', 'yml', 'ini'])('stylish %s check', (extension) => {
  expect(gendiff(getFixturePath(`filepath1.${extension}`), getFixturePath(`filepath2.${extension}`))).toBe(expectedStylish);
});
...
test.each(['json', 'yml', 'ini'])('json %s check', (extension) => {
